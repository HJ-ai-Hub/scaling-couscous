...
import { getProductsNavGroup, getSimpleNavLinks, getSolutionsNavGroup, type NavGroup } from "@/content/nav";
...
function LanguageSwitcher({ pathname, className }: { pathname: string; className?: string }) {
  ...
  return (
    <div className={cn("relative", className)} onMouseEnter={openNow} onMouseLeave={closeSoon}>
      <button
        className={cn(
          "flex items-center gap-1.5 rounded-full px-3 py-2 text-[0.9rem] font-medium text-ink transition-colors hover:bg-surface-alt",
          open && "bg-surface-alt",
        )}
        aria-expanded={open}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 8 }}
            transition={{ duration: 0.18, ease: [0.16, 1, 0.3, 1] }}
            className="absolute right-0 top-full z-50 mt-3 w-44 rounded-card border border-border bg-white p-2 shadow-lift"
          >
            {locales.map((loc) => (
              <Link
                key={loc}
                ...
                locale={loc}
                className={cn(
                  "block rounded-control px-3 py-2 text-sm transition-colors hover:bg-surface-alt",
                  loc === locale ? "font-semibold text-ink" : "text-ink-soft",
                )}
              >
                {localeLabels[loc]}
              </Link>
...
export function Navbar() {
  ...
  const productsNavGroup = getProductsNavGroup(locale);
  const solutionsNavGroup = getSolutionsNavGroup(locale);
        ...
        <div className="flex items-center gap-1 lg:hidden">
          ...
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            <SheetTrigger asChild>
              <button
                className="flex h-11 w-11 items-center justify-center rounded-full text-ink transition-colors hover:bg-surface-alt"
